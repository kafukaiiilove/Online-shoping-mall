<template>
  ...
  <template v-slot="scope">
    ...
    <el-button size="mini" type="success" plain @click="confirmSetDefault(scope.row)">
      设为默认地址
    </el-button>
  </template>
</el-table-column>
...
updateUserAddress(address, phone, name) {
  ...
  .then(response => {
    if (response.data === '默认地址、电话和姓名更新成功') {
      this.$message.success('地址、电话和姓名已更新');
      // 更新地址、电话、姓名的事件
      this.$eventBus.$emit('addressUpdated', { address, phone, name });
console.log('触发了地址更新事件', { address, phone, name });
